   ...
   public function __construct()
...

    {
        $peminjaman = DB::table("keranjang_peminjaman")
            ->join('barang_news', function ($join) {
                $join->on('keranjang_peminjaman.id_barang', '=', 'barang_news.id');
            })->get();
    ...
    public function store(Request $request)
    {
        ...
        for ($a=0; $a < count($request->id); $a++) { 
            $sin = DB::table('keranjang_peminjaman')->where('id_barang', $request->id[$a])->count();

            if ($sin == 0) {
    
                    $tes = DB::table('barang_news')->where('id', $request->id[$a])->first();
    ...
    public function update(Request $request)
    {
        ...
        //     ]);
        // }
        $tes = DB::table('barang_news')->where('id', $request->id_barang)->first();
